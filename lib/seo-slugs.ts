...
export function getAllSlugs(): string[] {
  ...
  // Category pages: /jobs/software-engineer
  CATEGORIES.forEach(c => slugs.push(c.slug));

  // Location pages: /jobs/remote
  LOCATIONS.forEach(l => slugs.push(l.slug));

  // Job type pages: /jobs/full-time
  JOB_TYPES.forEach(j => slugs.push(j.slug));

  // Category + Location combos: /jobs/software-engineer-remote
  CATEGORIES.forEach(c => {
    LOCATIONS.forEach(l => {
      slugs.push(`${c.slug}-${l.slug}`);
    });
  });

  // Category + Job Type combos: /jobs/software-engineer-full-time
  CATEGORIES.forEach(c => {
    JOB_TYPES.forEach(j => {
      slugs.push(`${c.slug}-${j.slug}`);
    });
  });
...
export function parseSlug(slug: string): {
  ...
  return {
    title: 'Jobs',
    description: 'Browse jobs on ApplyFirst.',
  };
}
