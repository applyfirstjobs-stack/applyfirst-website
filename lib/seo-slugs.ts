...
import { parseSlug, getAllSlugs, CATEGORIES, LOCATIONS, ATS_SOURCES } from '@/lib/seo-slugs';
...
export default async function BrowsePage({ params }: { params: { slug: string } }) {
  ...
  let query = supabase
    .from('jobs')
    .select('id,job_title,company_name,location,job_type,apply_score,date_posted,created_at,industry,salary,source')
  ...
  if (parsed.source) countQuery = countQuery.eq('source', parsed.source);
      ...
      <section className="border-b border-white/5 bg-[#050505]">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-14">
          ...
          <div className="flex items-center gap-4 flex-wrap">
            ...
            {parsed.source && (
              <span className="text-[#d4af37]/60 bg-[#d4af37]/5 border border-[#d4af37]/10 px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest">
                via {parsed.source}
              </span>
            )}
          </div>
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px] font-black uppercase tracking-widest text-white/30">
                      ...
                      {job.source && (<><span>·</span><span className="text-white/20">via {job.source}</span></>)}
                      <span>·</span>
      ...
      <section className="border-t border-white/5 bg-[#050505]">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-12">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {ATS_SOURCES.map((ats) => (
              <Link key={ats.slug} href={`/browse/${ats.slug}-jobs`}
                className="text-xs font-semibold px-4 py-2 rounded-full border border-[#d4af37]/10 text-[#d4af37]/50 hover:text-[#d4af37] hover:border-[#d4af37]/30 transition-all">
                {ats.label} Jobs
              </Link>
            ))}
          </div>
