...
export async function generateMetadata({ params }: { params: { slug: string } }) {
  ...
  return {
    ...
    alternates: {
      canonical: `https://www.applyfirstjobs.com/jobs/${params.slug}`,
    },
    openGraph: {
      title: `${parsed.title} — ApplyFirst`,
      description: parsed.description,
      url: `https://www.applyfirstjobs.com/jobs/${params.slug}`,
    },
  };
}
...
export default async function SEOJobPage({ params }: { params: { slug: string } }) {
  const parsed = parseSlug(params.slug);
      ...
      <section className="border-t border-white/5 bg-[#050505]">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-12">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {LOCATIONS.map((loc) => (
              <Link key={loc.slug} href={`/jobs/${loc.slug}`}
                className="text-xs font-semibold px-4 py-2 rounded-full border border-white/10 text-white/50 hover:text-white hover:border-[#d4af37]/30 transition-all">
                {loc.label} Jobs
              </Link>
            ))}
